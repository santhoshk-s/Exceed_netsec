...
import { fetchHelpMessages } from "../store/help/helpSlice";
import ContactModal from "./ContactModel"; // Import the modal component
import { IoOpen } from "react-icons/io5";
import HelpModel from "./HelpModel";

function HelpTable() {
  const dispatch = useDispatch();
  const { loading, helpMessages, error } = useSelector((state) => state.help);
  ...
  const sortedMessages = [...helpMessages].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    ...
    Help Data
        ...
        <nav className="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4" aria-label="Table navigation">
          <span className="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">
            Showing <span className="font-semibold text-gray-900 dark:text-white">{indexOfFirstMessage + 1}-{Math.min(indexOfLastMessage, helpMessages.length)}</span> of <span className="font-semibold text-gray-900 dark:text-white">{helpMessages.length}</span>
          </span>
